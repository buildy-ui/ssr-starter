...
const template = (payload: { html: string; meta: { title: string; description: string; canonical: string; ogTitle?: string; ogDescription?: string }; site: { lang?: string; charset?: string } }) => {
  const { html, meta, site } = payload;
...
<head>
  ...
  <script>window.__RENDER_CONTEXT__ = __CONTEXT__;</script>
