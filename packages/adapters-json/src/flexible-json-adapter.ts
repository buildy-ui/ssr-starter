...
import { DEFAULT_FLEXIBLE_JSON_PATH } from './paths';
...
export class FlexibleJsonAdapter implements FlexibleStorageAdapter {
  ...
  constructor(path = process.env.FLEXIBLE_JSON_PATH || DEFAULT_FLEXIBLE_JSON_PATH) {
  ...
  private async load(): Promise<JsonDbState> {
    ...
    if (!existsSync(this.filePath)) {
      this.state = { schemas: {}, collections: {}, meta: { lastModified: nowIso() } };
      return this.state;
    }
    ...
    const state: JsonDbState = {
      ...
      meta:
        isObject(parsed?.meta) && typeof parsed.meta.lastModified === 'string'
          ? parsed.meta
          : { lastModified: nowIso() },
    };
    ...
    for (const stage of pipeline) {
      ...
      if (stage.$lookup) {
        ...
        results = results.map((doc) => {
          ...
          const matches = foreignDocs.filter((f) => this.getNestedValue(f, stage.$lookup!.foreignField) === localValue);
